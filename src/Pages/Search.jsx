...
export default function Search() {
    ...
    // console.log(data.children[0].children[0].children[0].children[0])
    // for(var i = 0; i < data.children.length; i++) {
    //     for(var j = 0; j < data.children[i].children.length; j++) {
    //         for(var k = 0; k < data.children[i].children[j].children.length; k++) {
    //             for(var l = 0; l < data.children[i].children[j].children[k].children.length; k++) {
    ...
    //   const [pattern, setPattern] = useState('')
    //   const fuse = new Fuse(data, options);
    //   var matches = fuse.search(pattern[0])
    //   const fuse2 = new Fuse(matches, options);

    //   for(var i = 1; i < pattern.length; i++) {
    //     data = matches
    //     matches = fuse.search(pattern[i])
    return (
        <div className='flex items-center flex-col pt-24'>
            ...
            {matches.length>0 && <ul className='flex mt-3.5 w-[817px] flex-wrap gap-2 h-[750px] overflow-y-scroll'>
                ...
                <div className='h-[300px] w-[396px]'></div>
            </ul>}
        </div>
  );
}